...
def quantize_tensor(x, scale, zero_point, num_bits=8, signed=False):
    ...
    q_x.clamp_(qmin, qmax).round_()  # clamp：将输入input张量每个元素的夹紧到区间 [min,max]
                                     # 下划线表示是一个 in-place 类型函数
                                     # q=round(r/S+Z)
...
class QModule(nn.Module):
    ...
    def __init__(self, qi=True, qo=True, num_bits=8):
        # 指定量化的位数外，还需指定是否提供量化输入 (qi) 及输出参数 (qo)
        # 不是每一个网络模块都需要统计输入的 min、max，大部分中间层都是用上一层的 qo 来作为自己的 qi 的，
        # 另外有些中间层的激活函数也是直接用上一层的 qi 来作为自己的 qi 和 qo。
    ...
    def freeze(self):
        # 这个函数会在统计完 min、max 后，把这些项提前固定下来，同时也将网络的权重由浮点实数转化为定点整数。
        pass
...
class QConv2d(QModule):
    ...
    def __init__(self, conv_module, qi=True, qo=True, num_bits=8):
        # 传入了一个 conv_module 模块，这个模块对应全精度的卷积层，
